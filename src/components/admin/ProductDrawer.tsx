...
const Label = ({ children, className }: any) => (
  <label className={cn("text-[10px] font-black uppercase tracking-widest text-neutral-400 block mb-2", className)}>
    {children}
  </label>
);
...
const FormSection = ({ title, id, children }: { title: string; id: string; children: React.ReactNode }) => (
  <div id={id} className="scroll-mt-24 pt-16 first:pt-0 border-t first:border-t-0 border-neutral-100">
    <h3 className="text-2xl font-bold font-heading text-neutral-900 mb-8 flex items-center gap-3">
...
export default function ProductDrawer({
  ...
  return (
    <div className="fixed inset-0 z-[100] flex justify-end">
      ...
      {/* DRAWER */}
      <div className="relative w-full md:w-[70vw] lg:w-[60vw] h-full bg-white shadow-2xl flex flex-col md:flex-row overflow-hidden animate-in slide-in-from-right duration-500 font-sans">
        
        {/* SIDEBAR TABS */}
        <div className="hidden md:flex w-64 bg-neutral-50 border-r border-neutral-100 flex-col p-6 sticky top-0 h-full">
          <div className="mb-10 p-4 bg-white rounded-2xl border border-neutral-100 shadow-sm">
            <Label className="mb-1">Active Editor</Label>
            <div className="text-sm font-bold text-neutral-900 truncate">{form.name || "New Product"}</div>
          </div>
            ...
            {[
              ...
              <button 
                ...
                className={cn(
                  ...
                  activeTab === tab.id ? "bg-white shadow-md text-emerald-600" : "text-neutral-400 hover:bg-white/50 hover:text-neutral-900"
                )}
              >
              ...
              <FormSection title="Core Information" id="basic">
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                    <Field label="SKU / Ident">
                       <input value={form.sku || ""} onChange={e => updateForm({ sku: e.target.value })} placeholder="SKU-..." className="w-full h-14 px-6 bg-white border border-neutral-200 rounded-2xl outline-none font-bold" />
                    </Field>
                    <Field label="Base Price ($)" required>
                       <input type="number" value={form.price} onChange={e => updateForm({ price: e.target.value })} placeholder="0.00" className="w-full h-14 px-6 bg-white border border-neutral-200 rounded-2xl outline-none font-bold" />
                    </Field>
                    <Field label="Stock Status">
                       <input type="number" value={form.stock} onChange={e => updateForm({ stock: e.target.value })} placeholder="0" className="w-full h-14 px-6 bg-white border border-neutral-200 rounded-2xl outline-none font-bold" />
                    </Field>
                 </div>
                 <div className="flex items-center gap-3 p-4 bg-emerald-50 rounded-2xl border border-emerald-100">
                    <ShieldCheck className="h-5 w-5 text-emerald-600" />
                    <span className="text-xs font-bold text-emerald-800 uppercase tracking-widest">Mark as Authenticated / Verified Batch</span>
                    <button 
                      onClick={() => updateForm({ isAuthenticated: !form.isAuthenticated })}
                      className={cn("ml-auto size-12 rounded-xl flex items-center justify-center transition-all", form.isAuthenticated ? "bg-emerald-600 text-white" : "bg-neutral-200 text-neutral-400")}
                    >
                    ...
                    <Field label="Lifestyle Gallery">
                       <div className="grid grid-cols-3 md:grid-cols-5 gap-4">
                          {(form.galleryFiles || []).map((f: any, i: number) => (
                             <div key={i} className="relative aspect-square bg-neutral-100 rounded-2xl overflow-hidden border border-neutral-100 group">
                                ...
                                <button onClick={() => updateForm({ galleryFiles: form.galleryFiles.filter((_:any, idx:number) => idx !== i) })} className="absolute top-2 right-2 p-1.5 bg-black/60 text-white rounded-lg opacity-0 group-hover:opacity-100 transition-all"><X className="h-3 w-3" /></button>
                             </div>
                          ))}
                          <label className="aspect-square border-2 border-dashed border-neutral-200 rounded-2xl flex items-center justify-center text-neutral-300 cursor-pointer hover:border-emerald-500 transition-all font-sans text-[10px] font-bold uppercase tracking-widest flex-col gap-1">
                             <Plus /> Add
              ...
              <FormSection title="Product Variants" id="variants">
                 <div className="space-y-6">
                    {(form.variants || []).map((v: any, i: number) => (
                       <div key={i} className="p-8 bg-neutral-50 rounded-[2.5rem] border border-neutral-100 space-y-6 relative group">
                          <button onClick={() => updateForm({ variants: form.variants.filter((_:any, idx:number) => idx !== i) })} className="absolute top-6 right-6 p-2 text-neutral-300 hover:text-rose-500 opacity-0 group-hover:opacity-100 transition-all"><Trash2 className="h-4 w-4" /></button>
                          <div className="flex gap-8 items-start">
                             <label className="relative size-24 bg-white border border-neutral-100 rounded-2xl flex items-center justify-center cursor-pointer overflow-hidden">
                                {v.image ? <Image src={typeof v.image === 'string' ? `${process.env.NEXT_PUBLIC_API_URL}/images/${v.image}` : URL.createObjectURL(v.image)} fill className="object-cover" alt="V" /> : <Plus className="text-neutral-200" />}
                                <input type="file" className="hidden" onChange={e => {
                                  const n = [...form.variants]; n[i].image = e.target.files?.[0]; updateForm({ variants: n });
                                }} />
                             </label>
                             <div className="flex-1 grid grid-cols-2 md:grid-cols-4 gap-6">
                                <div className="space-y-1"><Label>Option Name</Label><input value={v.title} onChange={e => { const n = [...form.variants]; n[i].title = e.target.value; updateForm({ variants: n }); }} placeholder="Pack of 1..." className="w-full h-11 px-4 bg-white border border-transparent rounded-xl outline-none font-bold text-sm" /></div>
                                <div className="space-y-1"><Label>SKU</Label><input value={v.sku || ""} onChange={e => { const n = [...form.variants]; n[i].sku = e.target.value; updateForm({ variants: n }); }} className="w-full h-11 px-4 bg-white border border-transparent rounded-xl outline-none font-bold text-sm" /></div>
                                <div className="space-y-1"><Label>Price ($)</Label><input type="number" value={v.price} onChange={e => { const n = [...form.variants]; n[i].price = e.target.value; updateForm({ variants: n }); }} className="w-full h-11 px-4 bg-white border border-transparent rounded-xl outline-none font-bold text-sm" /></div>
                                <div className="space-y-1"><Label>Stock</Label><input type="number" value={v.stock} onChange={e => { const n = [...form.variants]; n[i].stock = e.target.value; updateForm({ variants: n }); }} className="w-full h-11 px-4 bg-white border border-transparent rounded-xl outline-none font-bold text-sm" /></div>
                             </div>
                          </div>
                       </div>
                    ))}
                    <button onClick={() => updateForm({ variants: [...form.variants, { title: "", sku: "", price: "", stock: 0, image: null }] })} className="w-full py-4 border-2 border-dashed border-neutral-100 rounded-3xl text-[10px] font-black uppercase text-neutral-300 hover:text-emerald-500 hover:border-emerald-500 transition-all font-sans">+ Add another variant</button>
                 </div>
              </FormSection>
                 ...
                 <div className="space-y-8">
                    <Field label="Detailed Page Description">
                       <textarea 
                        ...
                        className="w-full min-h-[120px] p-6 bg-white border border-neutral-200 rounded-2xl outline-none font-medium leading-relaxed" 
                       />
                    </Field>
                    ...
                    <div className="pt-8 border-t border-neutral-100 space-y-10">
                       <div className="flex items-center gap-3">
                          ...
                          <h4 className="text-base font-bold font-heading">Narrative & Highlights</h4>
                       </div>
                       <Field label="Section Header (e.g. 'The Precision Synthesis')">
                          <input value={form.subtitle} onChange={e => updateForm({ subtitle: e.target.value })} placeholder="Custom narrative header..." className="w-full h-14 px-6 bg-neutral-50 border border-transparent rounded-2xl outline-none font-bold italic" />
                       </Field>
                          ...
                          <div className="space-y-3">
                             {(form.contentSections || []).map((point: string, i: number) => (
                                <div key={i} className="flex gap-3">
                                   <div className="size-11 bg-neutral-100 rounded-xl flex items-center justify-center shrink-0"><Check className="h-4 w-4 text-emerald-500" /></div>
                                   <input 
                                     value={point} 
                                     onChange={e => { const n = [...form.contentSections]; n[i] = e.target.value; updateForm({ contentSections: n }); }}
                                     className="flex-1 h-11 px-6 bg-neutral-50 rounded-xl outline-none font-bold text-sm" 
                                   />
                                   <button onClick={() => updateForm({ contentSections: form.contentSections.filter((_:any, idx:number) => idx !== i) })} className="p-3 text-neutral-300 hover:text-rose-500"><Trash2 className="h-4 w-4" /></button>
                                </div>
                             ))}
              ...
              <FormSection title="Full Specifications" id="specs">
                 <div className="grid grid-cols-2 gap-4">
                    {(form.metadata || []).map((m: any, i: number) => (
                       <div key={i} className="flex gap-2 p-4 bg-neutral-50 rounded-2xl border border-neutral-100 group relative">
                          <button onClick={() => updateForm({ metadata: form.metadata.filter((_:any, idx:number) => idx !== i) })} className="absolute -top-2 -right-2 p-1.5 bg-white shadow-md rounded-full text-rose-500 border border-neutral-100 opacity-0 group-hover:opacity-100 transition-all"><X className="h-3 w-3" /></button>
                          <div className="flex-1 space-y-1">
                             <input value={m.key} onChange={e => { const n = [...form.metadata]; n[i].key = e.target.value; updateForm({ metadata: n }); }} className="w-full text-[10px] font-black uppercase text-neutral-400 bg-transparent outline-none" placeholder="Label..." />
                             <input value={m.value} onChange={e => { const n = [...form.metadata]; n[i].value = e.target.value; updateForm({ metadata: n }); }} className="w-full text-sm font-bold text-neutral-900 bg-transparent outline-none" placeholder="Value..." />
                          </div>
                       </div>
                    ))}
                    <button onClick={() => updateForm({ metadata: [...form.metadata, { key: "", value: "" }] })} className="h-24 border-2 border-dashed border-neutral-100 rounded-2xl flex items-center justify-center text-[10px] font-black uppercase text-neutral-300 hover:text-emerald-500 transition-all">+ Add Data Row</button>
                 </div>
              </FormSection>
                 ...
                 <div className="space-y-12">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                       <Field label="Page Title (SEO)">
                          <input value={form.metaTitle} onChange={e => updateForm({ metaTitle: e.target.value })} className="w-full h-14 px-6 bg-white border border-neutral-200 rounded-2xl outline-none font-bold" />
                       </Field>
                       <Field label="Canonical URL">
                          <input value={form.canonicalUrl} onChange={e => updateForm({ canonicalUrl: e.target.value })} className="w-full h-14 px-6 bg-white border border-neutral-200 rounded-2xl outline-none font-bold" />
                       </Field>
                    </div>
                    <Field label="Meta Description">
                       <textarea value={form.metaDescription} onChange={e => updateForm({ metaDescription: e.target.value })} className="w-full min-h-[100px] p-6 bg-white border border-neutral-200 rounded-2xl outline-none font-medium text-sm leading-relaxed" />
                    </Field>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                       <Field label="Keywords">
                          <input value={Array.isArray(form.keywords) ? form.keywords.join(", ") : form.keywords} onChange={e => updateForm({ keywords: e.target.value.split(",").map((s:string)=>s.trim()) })} className="w-full h-14 px-6 bg-white border border-neutral-200 rounded-2xl outline-none font-bold" />
                       </Field>
