                ...
                {order.items?.map((item: any, i: number) => (
                  <div key={item.id} className={cn(
                    ...
                    <div className="text-right">
                       <p className="font-bold">${(Number(item.price) * item.quantity / 100).toFixed(2)}</p>
                    </div>
                  </div>
                ))}
              </div>
           ...
           <section className="bg-[#062D1B] rounded-[2.5rem] p-10 text-white space-y-6 relative overflow-hidden">
              ...
              <div className="relative z-10 space-y-4">
                 <div className="flex justify-between text-white/40 text-sm font-bold uppercase tracking-widest">
                    <span>Subtotal</span>
                    <span className="text-white">${((Number(order.totalAmount) - Number(order.taxAmount) - Number(order.shippingAmount)) / 100).toFixed(2)}</span>
                 </div>
                 <div className="flex justify-between text-white/40 text-sm font-bold uppercase tracking-widest">
                    <span>Shipping</span>
                    <span className="text-white">${(Number(order.shippingAmount) / 100).toFixed(2)}</span>
                 </div>
                 <div className="flex justify-between text-white/40 text-sm font-bold uppercase tracking-widest">
                    <span>Taxes</span>
                    <span className="text-white">${(Number(order.taxAmount) / 100).toFixed(2)}</span>
                 </div>
                 <div className="h-px w-full bg-white/10 my-6" />
                 <div className="flex justify-between items-end">
                    <div>
                      <p className="text-[10px] font-black uppercase tracking-[0.4em] text-[#EBB56B] mb-2">Total Amount Paid</p>
                      <p className="text-4xl font-black tracking-tighter">${(Number(order.totalAmount) / 100).toFixed(2)}</p>
                    </div>
