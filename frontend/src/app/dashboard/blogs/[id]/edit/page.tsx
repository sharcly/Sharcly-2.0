...
export default function EditBlogPage() {
  ...
  useEffect(() => {
    const fetchBlog = async () => {
      try {
        ...
        const blog = response.data.blogs.find((b: any) => b.id === id);
        if (blog) {
          setFormData({
            title: blog.title || "",
            slug: blog.slug || "",
  ...
  if (fetching) {
    return (
      <div className="min-h-screen bg-[#FDFCFA] p-24 space-y-12">
  ...
  return (
    <div className="min-h-screen bg-[#FDFCFA] pb-24">
      ...
      <header className="sticky top-0 z-50 bg-white/80 backdrop-blur-3xl border-b border-gray-100 px-8 h-20 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
           <Button variant="ghost" asChild className="rounded-full px-6 font-bold text-xs"><Link href="/dashboard/blogs">Cancel</Link></Button>
             ...
             <AnimatePresence mode="wait">
                {activeTab === 'content' && (
                  <motion.div key="content" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-12">
                     <div className="space-y-4">
                        <Label className="text-xs font-bold text-gray-400 ml-4">Post Title</Label>
                        <Input 
                     ...
                     <div className="space-y-4">
                        <Label className="text-xs font-bold text-gray-400 ml-4">Content</Label>
                        <RichTextEditor 
                     ...
                     <div className="space-y-10">
                        ...
                        <div className="p-10 rounded-[3rem] bg-white border border-gray-100 shadow-sm space-y-8">
                           <div className="flex items-center gap-3 border-b border-gray-50 pb-4">
                              ...
                              <h3 className="text-lg font-bold">Post Settings</h3>
                           </div>
                           <div className="space-y-6">
                              <div className="space-y-2">
                        ...
                        <div className="p-10 rounded-[3rem] bg-white border border-gray-100 shadow-sm space-y-10">
                           ...
                           <div className="space-y-8">
                              ...
                              <div className="space-y-3">
                                 ...
                                 <Input value={formData.tags} onChange={(e) => setFormData({...formData, tags: e.target.value})} className="h-14 rounded-2xl border-gray-100 font-bold" />
                              </div>
                              <div className="space-y-3">
                                 <Label className="text-[10px] font-bold uppercase text-gray-400">Excerpt</Label>
                                 <Textarea value={formData.excerpt} onChange={(e) => setFormData({...formData, excerpt: e.target.value})} className="min-h-[160px] rounded-3xl border-gray-100 p-6 text-sm font-medium leading-relaxed resize-none" />
                              </div>
                           </div>
                        </div>
                     </div>
                  ...
                  <motion.div key="seo" initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} className="space-y-12 max-w-4xl mx-auto">
                     ...
                     <div className="p-10 rounded-[3rem] bg-[#062D1B] text-white space-y-10 shadow-2xl">
                        ...
                        <div className="space-y-4">
                           ...
                           <h4 className="text-2xl font-bold italic font-serif leading-snug">{formData.metaTitle}</h4>
                           <p className="text-sm opacity-50 leading-relaxed font-medium line-clamp-3">{formData.metaDescription}</p>
                        </div>
                     </div>
                  </motion.div>
                )}
